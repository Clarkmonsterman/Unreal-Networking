...
InputQueue::Init(int id, int input_size)
{
   ...
   //GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("Prediction Size: %d"), _prediction.size));
...
InputQueue::DiscardConfirmedFrames(int frame)
{
   ...
   //GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("Discard Frames?: %d"), frame - _last_added_frame));
...
InputQueue::AddDelayedInputToQueue(GameInput &input, int frame_number)
{
   ...
   int *intPointer = reinterpret_cast<int *>(input.bits);
   int predInt = *intPointer;

   if(_id == 1){
      //GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("Remote Add Input: %d"), predInt));
      //GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("Remote Add Frame: %d"), frame_number));
   }
   ...
   if (_prediction.frame != GameInput::NullFrame) {
      ...
      if (_first_incorrect_frame == GameInput::NullFrame && !_prediction.equal(input, true)) {
         ...
         //if(_id == 1){UE_LOG(LogTemp, Display, TEXT("First Incorrect Input: %d"), predInt);}
      }
